...
use_disposal_calc = st.sidebar.checkbox("🚨 啟動【處置預判】引擎 (較耗能)", value=False, help="預設關閉以確保量化篩選極速運行。建議先將標的篩選至少數幾檔後，再開啟此引擎進行歷史罰單比對與臨界點計算。")
        ...
        if selected_rows:
            ...
            with tab_disposal:
                if not use_disposal_calc:
                    st.info("⚠️ 您目前已關閉處置預判引擎。請先至左側面板【⚡ 進階運算設定】勾選啟動，以解鎖監視防護線與臨界點推演報告。")
                else:
