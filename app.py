...
if st.sidebar.button("📊 手動更新每日價量", type="primary", help="更新每日的股價、價量、本益比及三大法人等資料。\n\n⚠️ 更新頻率：每日"):
    try:
        ...
        url = f"https://api.github.com/repos/{GITHUB_USER}/{REPO_NAME}/actions/workflows/daily_crawler.yml/dispatches"
        headers = {
...
# 按鈕 3：注意與處置股 (warning_crawler.yml)
if st.sidebar.button("🚨 手動更新處置名單", type="primary", help="更新過去 120 天的注意與處置股歷史紀錄。\n\n⚠️ 更新頻率：每日傍晚"):
    try:
        GITHUB_USER = "redfish7956"
        REPO_NAME = "stock-scanner"
        GITHUB_TOKEN = st.secrets["GITHUB_TOKEN"]

        url = f"https://api.github.com/repos/{GITHUB_USER}/{REPO_NAME}/actions/workflows/warning_crawler.yml/dispatches"
        headers = {
            "Accept": "application/vnd.github+json",
            "Authorization": f"Bearer {GITHUB_TOKEN}",
            "X-GitHub-Api-Version": "2022-11-28"
        ...
        if res.status_code == 204:
            st.sidebar.success("✅ 成功觸發！雲端機器人已出動更新「注意與處置名單」。")
        else:
            st.sidebar.error(f"❌ 觸發失敗 ({res.status_code})")
    except Exception as e:
        st.sidebar.error(f"❌ 錯誤: 找不到 GITHUB_TOKEN 或設定異常。")
...
if not has_active_conditions:
    ...
    if not final_display_df.empty:
        ...
        if use_cond5: cols_to_color.append('最新EPS') # 啟動 EPS 篩選時標示藍字
